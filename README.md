...
단순히 "맞았다 / 틀렸다"를 보는 문제가 아닙니다. 실제 상황에서는 전체 제품 중 백오더가 발생하는 제품이 매우 적습니다. 따라서 모델이 모든 제품을 "백오더 아님"이라고 예측해도 accuracy는 높게 보일 수 있습니다. 하지만 그런 모델은 실제 업무에는 쓸 수 없습니다.
...
| `team_chunsang_v1/` | 박천상 팀원 참고 모델 | Base MLP와 Skip Connection MLP 실험입니다. 우리 `notebooks_v1/`, `notebooks_v2/`, `notebooks_v3/` 흐름과 구분하기 위해 별도 폴더로 분리했습니다. |
...
## 3. 왜 notebooks_v1, notebooks_v2, notebooks_v3로 나누었나
...
다만 이 폴더는 현재 우리 파이프라인과 전처리 기준이 완전히 같지 않습니다. 그래서 그대로 합치기보다는, 좋은 파생변수 아이디어만 가져와서 `notebooks_v2`에서 다시 정리했습니다.

### notebooks_v1: 내 1차 실험

`notebooks_v1/`은 현재 프로젝트에서 내가 만든 1차 공통 실험 폴더입니다. 최초 전처리 기준으로 XGBoost, LightGBM, MLP, FT-Transformer, TabNet, AutoEncoder, SMOTE, AE Hybrid를 비교했습니다.

이 폴더를 `notebooks_v1/`로 부르는 이유는, 이후 `notebooks_v2/`, `notebooks_v3/`와 같은 흐름으로 비교하기 위해서입니다.

### notebooks_v2: 팀원 파생변수와 참고 아이디어를 안전하게 반영

`notebooks_v2/`는 팀원들이 제안한 파생변수와 `team_chunsang_v1/`의 좋은 아이디어를 반영하되, **데이터 누수 없이 다시 학습한 버전**입니다.

여기서 중요한 원칙은 다음과 같습니다.

> train 데이터에서만 기준을 학습하고, validation/test에는 그 기준을 적용만 한다.

예를 들어 평균값, 중앙값, 스케일러, clip 기준, 최대값 같은 것은 전체 데이터를 보고 계산하면 안 됩니다. test 데이터의 정보를 미리 본 셈이 되기 때문입니다.
...
- 기존 MLP보다 더 좋은 딥러닝 모델을 만들 수 있는지 확인
...
예를 들어 모든 제품을 "백오더 아님"이라고 예측해도 약 99% 가까운 accuracy가 나올 수 있습니다. 하지만 실제 백오더 제품은 하나도 못 찾습니다.
...
말이 어렵지만, 이 프로젝트에서는 다음처럼 이해하면 됩니다.
...
이 모델은 A와 C처럼 실제 백오더인 제품을 앞쪽에 배치했습니다. 이 경우 좋은 모델입니다.

반대로 다음과 같은 모델은 좋지 않습니다.
...
ROC_AUC도 순위를 보는 지표입니다. 하지만 백오더처럼 Yes가 매우 적은 문제에서는 ROC_AUC가 지나치게 좋아 보일 수 있습니다.
...
XGB_SMOTE는 threshold 0.5 기준 F1은 좋아 보였지만, PR_AUC는 기본 XGBoost보다 낮아졌습니다. 즉 특정 기준점에서는 괜찮아 보일 수 있지만, 전체적인 위험 순위 품질은 떨어졌습니다.

### 6.7 Focal Loss

Focal Loss는 어려운 샘플에 더 집중하게 만드는 손실 함수입니다.

백오더 데이터에서는 대부분이 No입니다. 모델이 쉬운 No만 계속 맞히는 방향으로 학습하면, 희귀한 Yes를 잘 못 잡습니다.

Focal Loss는 모델이 이미 쉽게 맞히는 샘플의 영향은 줄이고, 잘 못 맞히는 샘플에 더 집중하게 합니다.
...
### 6.13 WeightedRandomSampler

WeightedRandomSampler는 학습 배치를 만들 때 희귀한 Yes 샘플이 더 자주 등장하도록 뽑는 방법입니다.

원래 데이터에서 Yes가 0.7%밖에 없으면, 미니배치 대부분이 No로 채워집니다. 그러면 딥러닝 모델은 Yes를 충분히 보지 못합니다.
...
### 7.3 V2에서 추가한 박천상님 참고 노트북 기반 안전 피처
...
### 8.3 `notebooks_v2/` 팀원 파생변수 기반 재실험
...
딥러닝 모델이 최종 1등은 아닙니다. 하지만 이것을 단순 실패로 보면 안 됩니다.

이 프로젝트에서는 다음을 확인했습니다.

1. 기본 MLP는 XGBoost보다 낮다.
2. Focal Loss, 불균형 샘플링, Residual 구조, Optuna 튜닝을 적용하면 MLP 성능이 실제로 오른다.
3. MLP_V3는 기존 MLP보다 PR_AUC를 약 +0.033 개선했다.
4. TabNet은 정형 데이터 특화 모델이지만, 이 데이터에서는 XGBoost나 MLP_V3보다 낮았다.
5. 정형 테이블 데이터에서는 여전히 GBDT 계열 모델이 강했다.

보고서에서는 다음처럼 정리하는 것이 좋습니다.
...
## 12. 팀원에게 설명할 때 쓸 수 있는 요약

### 짧은 버전

```text
현재 프로젝트는 백오더가 발생할 가능성이 높은 제품을 미리 찾는 문제입니다.
백오더가 전체 데이터에서 매우 적기 때문에 accuracy 대신 PR_AUC를 중심으로 모델을 비교했습니다.

V2에서는 팀원들이 제안한 파생변수와 박천상님 참고 노트북 아이디어를 누수 없이 반영해서 다시 학습했습니다.
그 결과 XGBoost가 PR_AUC 0.2959로 가장 높았습니다.

다만 수업이 딥러닝응용이기 때문에 V3에서는 머신러닝 모델을 더 튜닝하지 않고,
딥러닝 모델만 심화했습니다.

V3의 MLP는 Residual 구조, WeightedRandomSampler, Focal Loss, Optuna, PR_AUC 기준 Early Stopping을 적용했고,
기존 MLP 약 0.198에서 0.2305까지 성능이 올랐습니다.

최종 1등은 여전히 XGBoost지만,
딥러닝 모델도 고급 불균형 학습 기법을 적용하면 성능이 실제로 개선된다는 점을 확인했습니다.
```

### 발표용 버전

```text
저희 데이터는 백오더 Yes가 1%도 안 되는 매우 불균형한 데이터입니다.
그래서 단순 정확도보다는 백오더 위험 제품을 얼마나 앞쪽에 잘 배치하는지를 보는 PR_AUC를 주요 지표로 사용했습니다.

처음에는 기본 파생변수로 여러 모델을 비교했고,
이후 V2에서는 팀원들이 제안한 재고 여유, 실질 가용재고, 미래 가용재고, 부족률 같은 파생변수와
v1에서 나온 수요/재고 관련 피처를 누수 없이 추가했습니다.

V2에서는 XGBoost가 가장 높은 성능을 보였습니다.
하지만 딥러닝응용 과목 특성상, V3에서는 머신러닝 모델을 더 개선하지 않고 딥러닝 모델만 집중적으로 개선했습니다.

그 결과 MLP_V3는 기존 MLP보다 PR_AUC가 약 0.033 상승했습니다.
Residual MLP, Focal Loss, WeightedRandomSampler, Optuna 튜닝, PR_AUC 기준 Early Stopping이 효과를 보인 것으로 해석됩니다.

최종적으로 XGBoost가 가장 좋은 모델이지만,
딥러닝 모델도 문제 특성에 맞는 학습 전략을 적용하면 의미 있게 성능을 끌어올릴 수 있음을 확인했습니다.
```
